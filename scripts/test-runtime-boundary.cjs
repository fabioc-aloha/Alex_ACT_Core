...
test('Core bootstrap previews, applies, verifies, and repairs all canonical instructions', (t) => {
  ...
  assert.equal(preview.coreVersion, '3.0.1');
...
test('Core 3.0.1 release metadata is final before tagging', () => {
  ...
  assert.equal(manifest.distribution.published_version, '3.0.1');
  assert.equal(manifest.nextRelease, undefined);
  assert.equal(manifest.candidateVersion, undefined);
  assert.match(changelog, /## \[Unreleased\][\s\S]*## \[3\.0\.1\] - 2026-08-15[\s\S]*## \[3\.0\.0\] - 2026-08-15/);
  assert.match(readme, /published version.*3\.0\.1/i);
  assert.match(install, /\| Core \| `3\.0\.1` \|/);
