...
**Status in v0.7.0**: Core ships 42 skills, 14 slash-command prompts, and 18 source instructions. 17 load-bearing instructions are separately bootstrapped to user scope; 1 remains pattern-applied. Core intentionally ships zero agents. New content lands through evidence-gated proposals from `Alex_ACT_Steward` (top-of-chain in the plugin-architecture lineage).
