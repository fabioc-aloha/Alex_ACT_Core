...
Per-session meditation is one loop. Some projects find value in longer-cycle reviews:

- **Per release** — run meditation on what shipped; focus on lessons the release exposed
- **Per quarter (optional)** — audit accumulated learnings against project direction: did the patterns we've been extracting produce the outcomes we wanted?

Cadence is per-project. Adapt or skip based on the work rhythm.
...
- [ ] Future sessions can pick up where we left off
