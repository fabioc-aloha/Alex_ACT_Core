...
> Worker agents isolate mechanical work so Steward can keep judgment in the parent context.
...
Delegation is useful only when the worker has a valid tool contract and its isolation benefit exceeds dispatch overhead.
