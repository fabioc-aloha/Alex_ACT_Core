...
The six converter skills bundle executable muscles under `<skill>/scripts/<skill>.cjs` and depend on the shared runtime at `.github/scripts/shared/` (`tool-runner.cjs`, `markdown-preprocessor.cjs`, `mermaid-pipeline.cjs`, `data-uri.cjs`). Runtime prerequisites: **pandoc** on PATH (all 6), **mermaid-cli** on PATH (md-to-html + md-to-word when Mermaid present), **jszip** optional (md-to-word). `markdown-mermaid` reads `.github/config/brand-palette.json` (6-role semantic palette + typography) for its init directive and classDef vocabulary; the palette is shared with the illustrator plugin's svg-banner + illustrator agent + flint-chart. Banner generation itself lives in `Alex_ACT_Illustrator_Plugin`, not Core (un-ported 2026-07-30 per Fabio directive).
